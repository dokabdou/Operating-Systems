...
static void ReadAtVirtual(OpenFile* executable, int virtualaddr, int numBytes, int position, TranslationEntry* pageTable, unsigned numPages) {
	char buffer[numBytes];  // buffer to store the bytes read from the executable
	executable->ReadAt(buffer, numBytes, position);

	// backup old table
	TranslationEntry* backupTable = machine->currentPageTable;
	unsigned backupNumPage = machine->currentPageTableSize;

	// put "new" table in machine
	...
	// restore old table
	machine->currentPageTable = backupTable;
	machine->currentPageTableSize = backupNumPage;
}
#endif  // CHANGED
...
static void DrawArea(FILE* output, unsigned sections_x, unsigned virtual_x, unsigned y, unsigned blocksize, struct segment* segment, const char* name) {
	...
	fprintf(output,
	        ...
	        sections_x, y - end * blocksize, virtual_x - sections_x, (end - page) * blocksize);

	fprintf(output, "<text x=\"%u\" y=\"%u\" fill=\"#000000\" font-size=\"%u\">%s</text>\n", sections_x, y - page * blocksize, blocksize, name);
}

unsigned AddrSpace::Dump(FILE* output,
                         unsigned addr_x,
                         unsigned sections_x,
                         unsigned virtual_x,
                         ...
                         unsigned y,
                         unsigned blocksize) {
	unsigned ret = machine->DumpPageTable(output, pageTable, numPages, addr_x, virtual_x, virtual_width, physical_x, virtual_y, y, blocksize);
