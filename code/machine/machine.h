...
class Machine : public dontcopythis {
   public:
	...
	void DumpReg(FILE* output, int val, const char* name, const char* color, int ptr_x, int ptr_y, unsigned virtual_x, unsigned y, unsigned blocksize);
	// Dump a register
	void DumpRegs(FILE* output, int ptr_x, int ptr_y, unsigned virtual_x, unsigned y, unsigned blocksize);
...
#define CheckPointer(addr) ASSERT_MSG(addr >= PageSize && addr <= (int)machine->currentPageTableSize * PageSize, "\naddress %d does not make sense!\n", addr)
